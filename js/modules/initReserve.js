...
const renderMonth = (wrapper, data) => {
	const labels = data.map(month => {
		...
		label.innerHTML = `
			...
			<span class="radio__label">${new Intl.DateTimeFormat('ru-RU', {month: 'long'}).format(new Date(year, month))}</span>
		`;
		return label;
	});
	wrapper.append(...labels)
...
const renderDay = (wrapper, data, month) => {
	const labels = data.map(day => {
		...
		label.innerHTML = `
			...
			<span class="radio__label">${new Intl.DateTimeFormat('ru-RU', {month: 'long', day: 'numeric'}).format(new Date(year,month,day))}</span>
		`;
		return label;
	});
	wrapper.append(...labels)
